...
export const Logo = ({ showText, textProperties }: { showText?: boolean; textProperties?: string[] }) => {
  ...
  return (
    <div>
      ...
      >
        <div>
          <svg className="h-10 lg:h-12" viewBox="0 0 79 79" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {showText ? <span className={`font-scottel text-lg lg:text-2xl text-cms ${textProperties?.join(" ")}`}>Chill Mount Stays</span> : <></>}
      </div>
    </div>
  );
};
